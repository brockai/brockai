...
import tenantMappings from '../mappings/tenant_template.json';
import { Link } from 'react-router-dom';
import UserOne from '../images/user/user-01.png';

import { Toaster } from 'react-hot-toast';
import Loader from '../common/Loader';
import { fetchData, fetchStats, createIndex } from '../services/VectorService'
...
function Dataset() {
  ...
  useEffect(() => {
    ...
    const setTenant = async () => {
      const tenant =  await createIndex(tenantId, tenantMappings);
      
    };

    const fetchIndices = async () => {
      try {
        const indices = await fetchData('_cat/indices');
        const index = indices.indexOf(tenantId);
console.log(index)
        if (index === -1) {
           setTenant();
        } else {
          const stats = await fetchStats([tenantId]);
          console.log(stats)
        }
